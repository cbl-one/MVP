...
# Function to create slides in a PowerPoint presentation
...
def upload_file():
    ...
    # Step 3: Split the summary into multiple slides
    slides_data = []
    for slide_text in summary.split("\n\n"):  # Assuming AI separates slides with double newlines
        if "*" in slide_text:
            heading, content = slide_text.split("*", 1)  # Split at the first "*"
            slides_data.append({"heading": heading.strip(), "content": content.strip()})
        else:
            slides_data.append({"heading": slide_text.strip(), "content": ""})

    # Create output PowerPoint file
    output_ppt_path = os.path.join(temp_dir, "Summarized_Slides.pptx")
    create_ppt(slides_data, output_ppt_path)
